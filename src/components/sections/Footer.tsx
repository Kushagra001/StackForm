...
export function Footer() {
  ...
  return (
    <footer
      className="py-12 border-t border-sf-border"
      ...
      <div className="max-w-6xl mx-auto px-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <div
              className="text-sf-white-faint mt-2"
              style={{ fontSize: '11px', fontFamily: 'var(--font-jetbrains, monospace)' }}
            >
              {'// Currently building: stack-form.dev'}
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            {socials.map(({ href, label, svg }) => (
              <a
                ...
                aria-label={label}
                className="text-sf-white-faint hover:text-sf-white transition-colors"
              >
                {svg}
              </a>
            ))}
        ...
        <div className="flex items-center justify-between pt-6 border-t border-sf-border">
          <p className="text-sf-white-faint" style={{ fontSize: '11px' }} suppressHydrationWarning>
            © {new Date().getFullYear()} Stackform · stack-form.dev
          </p>
          <p className="text-sf-white-faint" style={{ fontSize: '11px' }}>
            Jaipur, India · Available worldwide
          </p>
        </div>
      </div>
    </footer>
